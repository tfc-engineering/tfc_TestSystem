...
class WordFloatCheck(CheckBase):
    '''Performs a check on a specific word of type float.'''
    @staticmethod
    def getInputParameters() -> InputParameters:
        ...
        params.addRequiredParam("word_number", ParameterType.INTEGER,
                                "Which word (zero-based index) to be compared to "
                                "the gold value.")
        params.addRequiredParam("gold_value", ParameterType.FLOAT,
                                "The float-value required to match.")
        params.addRequiredParam("tolerance", ParameterType.FLOAT,
                                "The tolerance to which the gold-value is evaluated.")
    ...
    def __init__(self, params: InputParameters) -> None:
        ...
        self.gold_value_ = params.getParam("gold_value").getFloatValue()
        self.tolerance_ = params.getParam("tolerance").getFloatValue()
    ...
    def executeCheck(self, config: dict, annotations: list[str]) -> bool:
        ...
        difference = relevant_value - self.gold_value_
        if abs(difference) > self.tolerance_:
            tol = self.tolerance_
...
PyFactory.register(WordFloatCheck, "WordFloatCheck")
